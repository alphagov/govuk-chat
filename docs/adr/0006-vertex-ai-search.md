...
## Context
The Search API V2[1] app uses Google Vertex AI Search[2] (VAIS) to index and retrieve GOV.UK content, whereas GOV.UK Chat uses an OpenSearch cluster. Both applications operate on the same content (i.e. published documents) and provide results based on a search query.s
...
- The Search API V2 datastore is not chunked and were it be a chunked one this would incur a performance penalty for Search API V2 operations[3]. Therefore we believe that in order for Search API V2 to support providing chunked GOV.UK content this would need a secondary data store and decisions for any interactions with a data store as to whether it should apply to one or both data stores.
