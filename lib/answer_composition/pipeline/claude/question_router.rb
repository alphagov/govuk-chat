  ...
  module Claude
    class QuestionRouter
      ...
      def call
        ...
        answer.assign_llm_response("question_routing", anthropic_response.to_h)
    ...
    private
      ...
      def llm_classification_function
        @llm_classification_function ||= anthropic_response[:content][0]
      end
      ...
      def anthropic_client
        @anthropic_client ||= Anthropic::BedrockClient.new(
          aws_region: ENV.fetch("CLAUDE_AWS_REGION", "eu-west-1"),
        )
      end
      ...
      def build_metrics(start_time)
        {
          ...
          llm_completion_tokens: anthropic_response[:usage][:output_tokens],
          llm_cached_tokens: anthropic_response[:usage][:cache_read_input_tokens],
        }
      end
    end
  end
