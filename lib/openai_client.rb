  ...
  private
    ...
    def record_rate_limit_metrics(env)
      ...
      if headers["x-ratelimit-remaining-tokens"] && headers["x-ratelimit-limit-tokens"]
        ...
        PrometheusMetrics.gauge("openai_remaining_tokens", remaining_tokens, { endpoint:, model: model_name })
        PrometheusMetrics.gauge("openai_tokens_used_percentage", ((tokens_limit - remaining_tokens) / tokens_limit) * 100, { endpoint:, model: model_name })
      end
      ...
      if headers["x-ratelimit-remaining-requests"] && headers["x-ratelimit-limit-requests"]
        ...
        PrometheusMetrics.gauge("openai_remaining_requests", remaining_requests, { endpoint:, model: model_name })
        PrometheusMetrics.gauge("openai_requests_used_percentage", ((requests_limit - remaining_requests) / requests_limit) * 100, { endpoint:, model: model_name })
      end
    end
  end
