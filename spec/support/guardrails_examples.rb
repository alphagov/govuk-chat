module GuardrailsExamples
  shared_examples "a passing guardrail pipeline step" do |guardrail_name|
    ...
    it "does not change the message" do
      expect { described_class.new(llm_provider: :openai).call(context) }.not_to change(context.answer, :message)
    end

    it "sets the status" do
      expect { described_class.new(llm_provider: :openai).call(context) }
