RSpec.describe AutoEvaluation::AnswerRelevancy do
  describe ".call" do
    ...
    let(:shared_expected_metrics_attributes) do
      {
        duration: 2.0,
    ...
    it "returns a results object with the expected attributes" do
      ...
      expected_llm_responses = {
        statements: bedrock_converse_client_response(content: statements_json).to_h,
        verdicts: bedrock_converse_client_response(content: verdicts_json).to_h,
        reason: bedrock_converse_client_response(content: reason_json).to_h,
      }
      expected_metrics = {
