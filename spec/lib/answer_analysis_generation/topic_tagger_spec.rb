RSpec.describe AnswerAnalysisGeneration::TopicTagger, :aws_credentials_stubbed do
  describe ".call" do
    ...
    context "when an answer analysis gets created for the answer during the LLM request" do
      it "raises an error" do
        client = Anthropic::BedrockClient.new(aws_region: ENV["CLAUDE_AWS_REGION"])
        allow(Anthropic::BedrockClient).to receive(:new) do
          create(:answer_analysis, answer:)
          client
        end
        stub_claude_messages_topic_tagger(question.message)
        ...
        expect { described_class.call(answer) }
          .to raise_error("Analysis already created for answer #{answer.id}")
      end
    end
  end
end
