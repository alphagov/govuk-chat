      ...
      shared_examples "evaluates guardrails" do |provider|
        it "runs successfully, outputs summary, and saves the results to a file with the given path" do
          ...
          begin
            ...
            expected_model = case provider&.to_sym
                             when :claude
                               Guardrails::Claude::MultipleChecker::BEDROCK_MODEL
                             else
                               Guardrails::OpenAI::MultipleChecker::OPENAI_MODEL
                             end
            expect(results["model"]).to eq(expected_model)
