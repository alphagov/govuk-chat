RSpec.describe Search::ResultsForQuestion, :chunked_content_index do
  describe "#self.call" do
    ...
    before do
      ...
      allow(described_class::Reranker).to receive(:document_type_weighting)
      allow(described_class::Reranker).to receive(:document_type_weighting).with("guide", "guide").and_return(1.0)
      allow(described_class::Reranker).to receive(:document_type_weighting).with("corporate_information_page", "about").and_return(0.3)
      allow(described_class::Reranker).to receive(:document_type_weighting).with("help_page", "help_page").and_return(0.4)
