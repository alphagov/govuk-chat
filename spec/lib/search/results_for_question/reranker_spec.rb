  ...
  describe ".call" do
    let(:chunked_content_results) do
      [
        ...
        build_chunked_content_result(score: 0.25, document_type: "answer"),
      ]
    end

    before do
      allow(described_class).to receive(:document_type_weightings).and_return(
        { "guide" => 4.0, "export_health_certificate" => 0.8, "answer" => 0.0 },
      )
    end
    ...
    it "returns results sorted by weighted_score" do
      expect(described_class.call(chunked_content_results).map { |r| [r.document_type, r.weighted_score] }).to eq(
        [
          ...
          ["answer", 0.0], # doc type weighting of 0.0
        ],
      )
    end
