RSpec.describe AnswerComposition::Pipeline::Claude do
  describe ".prompt_config" do
    it "fetches the prompt configuration for the given prompt name and model" do
      config = {
    ...
    it "raises an error if no prompt configuration is found for the given prompt name" do
      expect { described_class.prompt_config(:non_existent_prompt, :claude_sonnet_4_0) }
        .to raise_error("No LLM prompts found for non_existent_prompt")
    end
