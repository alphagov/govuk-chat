  ...
  describe ".config" do
    it "fetches the prompt configuration for the given component name and model" do
      ...
      allow(Rails.configuration
                 .govuk_chat_private
                 .llm_prompts.answer_composition)
                 .to receive(:fetch).with(:test_prompt).and_return({ claude_model: config })

      result = described_class.config(:test_prompt, :claude_model)

      expect(result).to eq(config)
    end

    it "raises an error if no prompt configuration is found for the given component name" do
      expect { described_class.config(:non_existent_component, :claude_model) }
    ...
    it "raises an error if no prompt configuration is found for the given model" do
      allow(Rails.configuration
                 .govuk_chat_private
                 .llm_prompts.answer_composition)
                 .to receive(:fetch).with(:test_prompt).and_return({ claude_model: {} })
