  ...
  context "when show_identifier is true" do
    it "omits the .govuk-visually-hidden class from the message identifier" do
      ...
      expect(rendered).to have_selector("li.app-c-conversation-message#answer-5") do |rendered_question|
        expect(rendered_question)
          .to have_selector(".app-c-conversation-message__identifier", text: "GOV.UK Chat")
        expect(rendered)
          .not_to have_selector(".app-c-conversation-message__identifier .govuk-visually-hidden", text: "GOV.UK Chat")
      end
    end
  end

  context "when show_identifier is false/not passed" do
    it "adds the .govuk-visually-hidden class to the message identifier" do
      render("components/conversation_message", {
        id: "answer-6",
        message: "message 6",
      })
