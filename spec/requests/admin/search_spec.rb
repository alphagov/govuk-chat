RSpec.describe "Admin::SearchController", :aws_credentials_stubbed, :chunked_content_index do
  describe "GET :index" do
    before do
      allow(Search::ResultsForQuestion::Reranker).to receive(:document_type_weighting).with("guide", "guide").and_return(1.2)
      allow(Search::ResultsForQuestion::Reranker).to receive(:document_type_weighting).with("answer", "answer").and_return(0.5)
    ...
    context "with search_text in params" do
      ...
      context "when there are chunks found" do
        ...
        it "includes score calculation and back links in links to results" do
          ...
          document_type_weight = Search::ResultsForQuestion::Reranker.document_type_weighting(chunk_to_find[:document_type], chunk_to_find[:schema_name])
